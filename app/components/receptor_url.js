...
var ReceptorUrl = React.createClass({
  ...
  render() {
    ...
    return (
      <form className="receptor-url" action="/setup" method="POST" onSubmit={this.submit} ref="form">
        <FormGroup className="man" onValidate={this.validate} ref="receptorUrl">
          <label className="man">
            ...
            <input className="form-control type-ellipsis-1-line" name="receptor_url" value={receptorUrl} onFocus={this.focus} onChange={this.change} onBlur={this.blur}/>
          </label>
        </FormGroup>
      </form>
    );
