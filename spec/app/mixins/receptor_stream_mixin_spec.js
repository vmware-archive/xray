...
describe('ReceptorStreamMixin', function() {
  ...
  beforeEach(function() {
    ...
    actualLrp = Factory.build('actualLrp', {
      ...
      process_guid: 'xyz',
      index: 1,
      modification_tag: {epoch: 1, index: 1}
    });
    ...
    AuthorizationApi = require('../../../app/api/authorization_api');
    authorizationDeferred = new Deferred();
    spyOn(AuthorizationApi, 'create').and.returnValue(authorizationDeferred);
  });
  ...
  describe('#createSSE', function() {
    beforeEach(function() {
      subject.createSSE(receptorUrl);
    });

    it('calls authorization create', function() {
      expect(AuthorizationApi.create).toHaveBeenCalled();
    });

    describe('when the authorization api is successful', function() {
      beforeEach(function() {
        authorizationDeferred.resolve();
      ...
      describe('#streamActualLrps', function() {
          ...
          describe('when the index for the changed lrp does not exist', function() {
            ...
            it('removes the old actual lrp from its old index', function() {
              var $receptor = new Cursor({
                ...
                desiredLrpsByProcessGuid: {},
                actualLrpsByProcessGuid: {},
                actualLrpsByCellId: {[actualLrp.cell_id]: [actualLrp]}
              }, callbackSpy);
              subject.setProps({$receptor});
        ...
        describe('when an actual_lrp change event is received', function() {
          ...
          beforeEach(function() {
            changedActualLrp = Factory.build('actualLrp', {
              cell_id: 'android17',
              process_guid: 'xyz',
              index: 1,
              modification_tag: {epoch: 1, index: 2}
            });
          });
          ...
          describe('when the index for the changed lrp does not exist', function() {
            ...
            it('removes the old actual lrp from its old index', function() {
              var $receptor = new Cursor({
                ...
                desiredLrpsByProcessGuid: {},
                actualLrpsByProcessGuid: {},
                actualLrpsByCellId: {[changedActualLrp.cell_id]: [changedActualLrp]}
              }, callbackSpy);
              subject.setProps({$receptor});
  ...
  describe('#streamSSE', function() {
    beforeEach(function() {
      spyOn(subject, 'streamDesiredLrps').and.callThrough();
      spyOn(subject, 'streamActualLrps').and.callThrough();
      subject.streamSSE(receptorUrl);
    });

    describe('when the authorization is not successful', function() {
      beforeEach(function() {
        authorizationDeferred.reject();
      });
